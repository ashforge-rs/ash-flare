...
impl PySupervisorSpec {
    ...
    fn with_restart_intensity(&mut self, intensity: &PyRestartIntensity) -> PyResult<()> {
        self.inner.restart_intensity = intensity.inner.clone();
        Ok(())
    }
    ...
    fn add_supervisor(&mut self, supervisor: &PySupervisorSpec) -> PyResult<()> {
        self.inner = std::mem::replace(&mut self.inner, SupervisorSpec::new("temp"))
...
impl PySupervisorHandle {
    ...
    fn count_children(&self, py: Python<'_>) -> PyResult<Py<PyDict>> {
        ...
        match result {
            Ok(children) => {
                let supervisors = children.iter().filter(|c| matches!(c.child_type, ChildType::Supervisor)).count();
                let workers = children.iter().filter(|c| matches!(c.child_type, ChildType::Worker)).count();
                let dict = PyDict::new(py);
                dict.set_item("supervisors", supervisors)?;
                dict.set_item("workers", workers)?;
                Ok(dict.into())
            },
            Err(e) => Err(PyRuntimeError::new_err(format!("Failed to count children: {}", e)))
        }
    }
